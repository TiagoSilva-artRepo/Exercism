...
public class SecurityPassMaker
{
    ...
    {
        if (support is Staff staff) return GetDisplayNameForStaff(staff);
        return "Too Important for a Security Pass";
    }

    private string GetDisplayNameForStaff(Staff staff)
    {
        if (staff is Security security) return GetDisplayNameForSecurity(security);
        return staff.Title;
    }

    private string GetDisplayNameForSecurity(Security security)
    {
        if (security.GetType() == typeof(SecurityJunior) || security.GetType() == typeof(SecurityIntern) || security.GetType() == typeof(PoliceLiaison)) return security.Title;
